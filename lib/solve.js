...
function exploreNextLocation(maze, canvas, state, colorBacktracked) {
    var s = state.solve;
    ...
    if (next) {
        ...
        s.prev = s.curr;
        s.curr = next;
        maze[s.curr.y][s.curr.x].visited = utils.yes;
    }
    ...
    paintRoute(maze, canvas, state, colorBacktracked);
}
...
function paintRoute(maze, canvas, state, colorBacktracked) {
    var s = state.solve;
