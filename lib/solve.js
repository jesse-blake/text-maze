...
module.exports = {
    ...
    run: function(app, doReset) {
        var s = app.solve;

        if (app.solve.showingSolution) {
            controls.updateCtrlText('show-maze-solution-ctrl', 'Show Solution');
            app.solve.showingSolution = false;
        }
    ...
    show: function(app) {
        ...
        app.solve.showingSolution = !app.solve.showingSolution;

        reset(app);

        if (app.solve.showingSolution) {
            while (!atEnd(app.maze, s.curr.x, s.curr.y)) {
                exploreNextLocation(app, false);
            }
            controls.updateCtrlText('show-maze-solution-ctrl', 'Hide Solution');
        }
        else {
            controls.updateCtrlText('show-maze-solution-ctrl', 'Show Solution');
        }
...
function exploreNextLocation(app, colorBacktracked) {
    var s    = app.solve;
    ...
    paintRoute(app, colorBacktracked);
}
...
function paintRoute(app, colorBacktracked) {
    var s    = app.solve;
    var maze = app.maze;
