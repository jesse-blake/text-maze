...
var CharMaps = require('./build/CharacterMaps.js');
...
module.exports = function(maze, canvas, state) {
    ...
    var mazeTextInputCtrl     = document.getElementById('maze-text-input-ctrl');
    var mazeResetCtrl         = document.getElementById('maze-reset-ctrl');

    // Auto resize maze when window size changes.
    ...
    // Build a new maze.
    if (mazeRebuildCtrl) {
        addListener('click', mazeRebuildCtrl, function(evt) {
            maze = rebuild(maze, canvas, state);
            stopDefault(evt);
        });
    }
    ...
    if (mazeShowTextCtrl) {
        addListener('click', mazeShowTextCtrl, function(evt) {
    ...
    if (mazeShowSolutionCtrl) {
        addListener('click', mazeShowSolutionCtrl, function(evt) {
    ...
    if (mazeSolveCtrl) {
        addListener('click', mazeSolveCtrl, function(evt) {
    ...
    if (mazeSolveResetCtrl) {
        addListener('click', mazeSolveResetCtrl, function(evt) {
    ...
    if (mazeDecreaseSpeedCtrl) {
        addListener('click', mazeDecreaseSpeedCtrl, function(evt) {
            ...
            if (solveSpeed > 1) {
                ...
                if (state.solve.running) {
                    ...
                    state.solve.delay = state.solve.animationDelays[solveSpeed - 2];
                    solve.run(maze, canvas, state);
                }
                else {
                    mazeCurrentSpeed.innerHTML = solveSpeed - 1;
    ...
    if (mazeCurrentSpeed) {
        mazeCurrentSpeed.innerHTML = state.solve.animationDelays.indexOf(state.solve.delay) + 1;
    }

    // Increase the speed of the solve animation.
    if (mazeIncreaseSpeedCtrl) {
        addListener('click', mazeIncreaseSpeedCtrl, function(evt) {

            // Solve speeds are one more than the index of the current animation delay.
            var solveSpeed = parseInt(mazeCurrentSpeed.innerHTML);

            if (solveSpeed < state.solve.animationDelays.length) {
                ...
                if (state.solve.running) {
                    solve.run(maze, canvas, state);
                    mazeCurrentSpeed.innerHTML = solveSpeed + 1;
                    state.solve.delay = state.solve.animationDelays[solveSpeed];
                    solve.run(maze, canvas, state);
                }
                else {
                    mazeCurrentSpeed.innerHTML = solveSpeed + 1;
    ...
    if (mazeDecreaseSizeCtrl) {
        addListener('click', mazeDecreaseSizeCtrl, function(evt) {
    ...
    if (mazeAutoSizeCtrl) {
        addListener('click', mazeAutoSizeCtrl, function(evt) {
    ...
    if (mazeIncreaseSizeCtrl) {
        addListener('click', mazeIncreaseSizeCtrl, function(evt) {
    ...
    // Change maze text according to user input.
    if (mazeTextInputCtrl) {
        mazeTextInputCtrl.oninput = function(evt) {
            var text = CharMaps.cleanText(mazeTextInputCtrl.value);

            clearTimeout(state.changeTextRebuildTimeout);

            if (text) {
                state.changeTextRebuildTimeout = setTimeout(function() {
                    state.text = mazeTextInputCtrl.value;
                    load.autoSetLocationSize(state);
                    maze = rebuild(maze, canvas, state);
                }, 300);
            }
        ...
        mazeTextInputCtrl.onpropertychange = mazeTextInputCtrl.oninput;
    }

    // Reset maze to original text.
    if (mazeResetCtrl) {
        addListener('click', mazeResetCtrl, function(evt) {

            // Clear the custom maze text input box.
            if (mazeTextInputCtrl.value) {
                mazeTextInputCtrl.value = '';
            }
