...
module.exports = function(text, maxMazeWidth, componentSize) {
    ...
    // Clean the string of text rendered in the maze
    function cleanText(text) {
        return text
            .replace(/(^\s+|\s+$)/g, '')    // Trim whitespace
            .replace(/[^A-Za-z0-9 ]/g, '') // Restrict to a whitelist of characters
    ...
    function lengthOfCharInMaze(ch) {
        return mazeChars['ch' + ch.toUpperCase()][0].length;
    }

    // Returns the number of maze components the word will require horizontally in the maze
    // including a single component between each character
    function getWordLength(word) {
        var i, length = 0, wordChars = word.split('');

        for (i = 0; i < wordChars.length; i++) {
            length += lengthOfCharInMaze(wordChars[i]);
    ...
    function hyphenateWord(lines, word, state, maxLineLen) {
        ...
        // The length (in maze components) the next character will require in the maze
        var nextLen;
        ...
        for (i = 0; i < chars.length; i++) {
            ...
            nextLen = lengthOfCharInMaze(chars[i]) + 1;
    ...
    var words = cleanText(text).split(' ');

    // Number of maze components the current word will require horizontally
    var wordLen;
    
    // Max length (in maze components) of a line of text in the maze
    var maxLineLen = maxMazeWidth / componentSize;

    // Length (in maze components) of word spacing
    ...
    var state = {
        ...
        // Current length (in maze components) of the current line of text
        lineLen: 0
    };
