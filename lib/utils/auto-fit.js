...
module.exports = function(text, mazeMargin) {
    ...
    // Set a default margin if none given (using bootstrap-4's sm min).
    mazeMargin = mazeMargin || (window.innerWidth < 576 ? 30 : 100);
    ...
    var height = computeMazeHeight(lines, cellSize);

    while (height <= MAX_MAZE_HEIGHT && width >= MIN_MAZE_WIDTH) {
        cellSize++;

        width  = getMaxMazeWidth(cellSize, mazeMargin);
        lines  = utils.breakTextIntoLines(text, width);
        height = computeMazeHeight(lines, cellSize);
    }
