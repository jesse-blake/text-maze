...
module.exports = {
    ...
    one: function(maze, canvas, state, x, y) {
        ...
        if (onPath(maze, x, y)) {
            ...
            if (onPath(maze, x, y-1)) {
                color = getConnectingColor(maze, x, y-1, maze[y][x].color);
                paint(canvas, color, x*ls+hb+tm, y*ls, ls-2*hb-2*tm, hb+tm);
                // paint(canvas, color, x*ls+hb, y*ls, ls-2*hb, hb);
            }
            if (onPath(maze, x+1, y)) {
                color = getConnectingColor(maze, x+1, y, maze[y][x].color);
...
function getConnectingColor(maze, x, y, defaultColor) {
    if (maze[y][x].color === 'lightgray') {
        return 'lightgray';
    }

    if (maze[y][x].color === 'white') {
        return 'white';
    }
