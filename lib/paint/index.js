...
module.exports = {
    ...
    one: function(maze, canvas, state, x, y) {
        var clr, fluctuate, onPathAbv, onPathRgt, onPathBlw, onPathLft;
        ...
        if (onPath(maze, x, y)) {
            ...
            // Paint borders.
            if (!onPathAbv) {
                paint(canvas, clr, x*ls, y*ls-bd, ls, bd);
            }
            if (!onPathRgt) {
                paint(canvas, clr, x*ls+ls, y*ls, bd, ls);
            }
            if (!onPathBlw) {
                paint(canvas, clr, x*ls, y*ls+ls, ls, bd);
            }
            if (!onPathLft) {
                paint(canvas, clr, x*ls-bd, y*ls, bd, ls);
            }

            // Paint border corners.
            if (!onPathAbv && !onPathRgt) {
                paint(canvas, clr, x*ls+ls, y*ls-bd, bd, bd);
            }
            if (!onPathRgt && !onPathBlw) {
                paint(canvas, clr, x*ls+ls, y*ls+ls, bd, bd);
            }
            if (!onPathBlw && !onPathLft) {
                paint(canvas, clr, x*ls-bd, y*ls+ls, bd, bd);
            }
            if (!onPathLft && !onPathAbv) {
                paint(canvas, clr, x*ls-bd, y*ls-bd, bd, bd);
            }
