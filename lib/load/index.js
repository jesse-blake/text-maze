...
module.exports = {
    ...
    // Initialize and register listeners for controls.
    controls: function(maze, canvas, state) {
        controls(maze, canvas, state, this.canvas, this.setLocationSize, this.autoSetLocationSize);
    },
...
function validateOpts(opts) {
    var i, o, optNames = ['text', 'locationSize', 'currentlyShowingText', 'animationSpeed', 'colors'];
...
function loadSolveState(opts, state) {
    ...
    // The solve speed corresponds to 1 + the index of the current animation delay.
    if ('animationSpeed' in opts) {
        if (Number.isInteger(opts.animationSpeed) && opts.animationSpeed > 0 && opts.animationSpeed <= state.solve.animationDelays.length) {
            state.solve.speed = state.solve.animationDelays[parseInt(opts.animationSpeed) - 1];
        }
        else {
            throw new Error("Maze option 'animationSpeed' must be an integer from 1 to " + state.solve.animationDelays.length + " inclusive");
        }
    }
    else {
        state.solve.speed = state.solve.animationDelays[4];
    }
}
