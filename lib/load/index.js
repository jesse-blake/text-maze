...
module.exports = {
    ...
    opts: function(opts) {
        ...
        // Wait a moment before rebuilding, in case more input is coming.
...
function validateOpts(opts) {
    ...
    if (typeof opts !== 'object') {
        throw new Error("Maze options must be defined in an object.");
    }
...
function loadSolveState(opts, state) {
    ...
    if ('animationDelay' in opts) {
        if (Number.isInteger(opts.animationDelay) && opts.animationDelay >= 0) {
            state.solve.animationDelay = opts.animationDelay;
        }
        else {
            throw new Error("Maze option 'animationDelay' must be an integer > 0.");
        }
    }
    else {
        state.solve.animationDelay = 20;
    }
}
